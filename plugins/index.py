...
async def index_files(bot, query):
    ...
    if raju == 'reject':
        ...
        await bot.send_message(int(from_user), f'Your Submission for indexing {chat} has been decliened by our moderators [@BEAST_TAMIL_MOVIE_65].', reply_to_message_id=int(lst_msg_id))
        return
    ...
    if int(from_user) not in ADMINS:
        await bot.send_message(int(from_user), f'Your Submission for indexing {chat} has been accepted by our moderators [@BEAST_TAMIL_MOVIE_65] and will be added soon.', reply_to_message_id=int(lst_msg_id))
    await msg.edit(
        "Starting Indexing",
...
async def send_for_index(bot, message):
    ...
    await message.reply('ThankYou For the Contribution, Wait For My Moderators [@BEAST_TAMIL_MOVIE_65] to verify the files.')
