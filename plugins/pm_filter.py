...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("oKda..പോയി നിന്റെ പണി നോക്ക്...🙄", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if files:
        for file in files:
            ...
            btn.append(
               [InlineKeyboardButton('🌻JOIN MY GROUP 🌻', url='http://t.me/Movie360group')
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('Check PM, I have sent files in pm ★ ഞാൻ PM ആയി അയച്ചിട്ടുണ്ട്, പോയി നോക്ക് 😋',show_alert = True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("I Like Your Smartness, But Don't Be Oversmart 😒ആ താഴെ കാണുന്ന ചാനലിൽ ഒന്ന് ജോയിൻ ചെയ്തൂടെ😉",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files = (await get_file_details(file_id))[0]
        title = files.file_name
        ...
        buttons = [[
            InlineKeyboardButton('✴️ Manuel Filter', callback_data='manuelfilter'),
        ...
        buttons = [[
            InlineKeyboardButton('👩‍🦯 Back', callback_data='help'),
            InlineKeyboardButton('Refreash ♻️', callback_data='rfrsh')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
